...
#### Product 페이지
- 제품설명 이미지 단일 → 다중 슬라이드 방식으로 확장
  - `Product` 타입에 `imageUrls?: string[]` 필드 추가 (기존 `imageUrl` 하위 호환 유지)
  - 라이트박스에 좌/우 화살표 네비게이션 + `1 / N` 카운터 추가
  - 이미지가 1장이면 기존과 동일하게 동작
- 2D GIS Engine 제품설명 이미지 2장으로 업데이트 (`product_img01.png`, `product_img01_01.jpg`)

#### 관리자 Product 폼
- 제품설명 이미지 URL 다중 입력 UI 추가
  - `+ 추가` 버튼으로 이미지 URL 항목 추가
  - X 버튼으로 개별 삭제
  - 저장 시 `imageUrls` 배열로 `product.json`에 반영
...
- `globals.css` body에 `word-break: keep-all` 전역 적용
  - 한국어 단어/어절 단위로 줄바꿈 (한 글자가 혼자 떨어지는 현상 방지)
  - `overflow-wrap: break-word` 병행 적용 (긴 영문 단어 예외 처리)
  - 모든 페이지 자동 적용
...
- 그리드 레이어 아래에 배경 이미지 레이어 삽입 (cover + center center)
